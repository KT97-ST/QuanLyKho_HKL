...
{
    public partial class UcHoaDonXuat_DaiLy : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public UcHoaDonXuat_DaiLy(int idUser, bool qt, bool qs, bool qx)
        {
            InitializeComponent();
        ...
        private void LoadTT()
        {
           ...
           // CreateSoHoaDon();
            ...
            setDef();
            gcDsSanPham.DataSource = ClsC_HoaDonXuat.Instance.GetDanhSachSanPham(Convert.ToDateTime(txtNgayLap.EditValue.ToString()), 1);
        }
        ...
        {
            gcDsHoaDon.DataSource = ClsC_HoaDonXuat.Instance.GetDanhSachHoaDon(1);
        }
        ...
        private void gvDsSanPham_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (int rowHandle in selectedRows)
                {
                    ...
                    GIAXUATDL = Convert.ToDecimal(gvDsSanPham.GetRowCellValue(rowHandle, "GIAXUATDL").ToString());
                }
            }catch(Exception ex)
            {
            ...
            //        {
            //            dtHoaDonChiTiet.Rows[i]["THANHTIEN"] = Convert.ToDecimal(dtHoaDonChiTiet.Rows[i]["SOLUONG"]) * Convert.ToDecimal(dtHoaDonChiTiet.Rows[i]["GIAXUATDL"]);
            //        }
            //    }
            //}
        ...
        private void Save_HD()
        {
            ...
            else if (_status == "st_edit")
            {
                try
                {
                    ...
                    for (int i = 0; i <= gvHoaDonXuatCT.RowCount; i++)
                    {
                        ...
                        {
                            isTrue = ClsC_HoaDonXuat.Instance.InsertHoaDonXuat_CT(idHoaDonNhapSelect, idSanpham, soLuong, GIAXUATDL, thanhTien, Convert.ToDateTime(txtNgayLap.EditValue.ToString()));
                        }
                    }
                    if (ret > 0)
                    {
            ...
            //foreach (DataRow dt in dtHoaDonChiTiet.Rows)
            //{
            //    // ClsC_HoaDonXuat.Instance.InsertHoaDonNhap_CT(Getidhoadonnhap, dt["SANPHAM_ID"].ToString(), dt["SOLUONG"].ToString(), dt["GIAXUATDL"].ToString(), dt["THANHTIEN"].ToString(), txtNgayLap.EditValue.ToString());
            //}
            //stt++;
            //dtHoaDonChiTiet.Clear();
            ////UcHoaDonNhap uc = new UcHoaDonNhap();
        ...
        private void txtNgayLap_EditValueChanged(object sender, EventArgs e)
        {
            try
           {
                gcDsSanPham.DataSource = ClsC_HoaDonXuat.Instance.GetDanhSachSanPham(Convert.ToDateTime(txtNgayLap.EditValue.ToString()), 1);//bán sỉ là 1
            }
            catch
            {
